...
4. 为每台目标设备确认唯一的 WireGuard `tunnelIp` 分配结果（例如 `10.0.0.2/32`、`10.0.0.3/32`）；未形成完整的 `deviceId + peerPublicKey + tunnelIp + lanCidr` 前，不得进入服务端部署。
...
7. 执行 WG 客户端配置，直接消费既有密钥、路由规划结果、已确认的 `tunnelIp` 分配结果，以及服务端公网 IP/端口。
