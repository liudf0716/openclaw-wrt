...
### 标准警告语（使用 shell 接口前必须逐字展示）

> ⚠️ **高风险操作警告**
>
> 即将在路由器上执行以下 Shell 命令：
> ```
> <完整命令>
> ```
> **潜在风险**：此命令将直接操作路由器系统，可能影响网络接口、路由表或防火墙规则。若执行出错，**可能导致网络立即中断，且无法通过远程工具恢复，需要物理接触路由器才能修复**。
>
> 请确认您已知晓上述风险，是否继续执行？（请回复「确认执行」或「取消」）
