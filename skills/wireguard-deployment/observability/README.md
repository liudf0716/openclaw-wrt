...
|------|---------------|-----------------|
| 1. Tunnel config exists | `clawwrt_get_wireguard_vpn` | `interface.proto = wireguard`, peers listed |
| 2. Tunnel is up with handshake | `clawwrt_get_wireguard_vpn_status` | `latest handshake` within last 2 minutes |
| 3. Routes are applied | `clawwrt_get_vpn_routes` | Expected CIDRs listed as `proto static` on `wg0` |
| 4. Data plane works | `clawwrt_get_wireguard_vpn_status` | RX/TX bytes increasing |
...
|---------|-------------|------------|-----|
| No handshake | Endpoint or firewall issue | `clawwrt_get_wireguard_vpn_status` shows no handshake | Verify `endpoint_host`, `endpoint_port`, and server firewall (UDP 51820) |
| Handshake OK but no traffic | Routes missing | `clawwrt_get_vpn_routes` returns empty | Re-apply routes via `clawwrt_set_vpn_routes` |
...
| Routes disappear after reboot | Routes are kernel-only | Routes gone after router reboot | Re-push routes via `clawwrt_set_vpn_routes` after tunnel comes up |
...
Prefer API tools. Use shell (`clawwrt_execute_shell`) only when API output is insufficient:
