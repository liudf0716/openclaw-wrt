...
7. 若当前是自动组网流程，必须先完成 `references/lan-collection.md`，并确保该模块已经为所有待接入客户端生成 WireGuard 密钥对；其中的 LAN 规划结果和客户端公钥共同作为服务端 peer AllowedIPs/peerBindings 配置的唯一依据，未完成前不允许进入 `openclaw_deploy_wg_server`。
