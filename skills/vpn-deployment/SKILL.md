---
name: vpn-deployment
description: End-to-end WireGuard VPN deployment guide — VPS server setup, router client configuration, and traffic routing via openclaw-wrt tools.
user-invocable: true
---
...
These are local shell commands to run on the VPS directly (not through openclaw-wrt tools).
...
Use openclaw-wrt tools to configure the router. Run these via the OpenClaw agent.
