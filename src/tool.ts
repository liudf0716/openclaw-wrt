...
const CollectWireguardProtectedRoutesSchema = Type.Object(
  {
...
function extractWireguardConfigSnapshot(response: JsonRecord): {
  privateKey?: string;
  addresses: string[];
  peerPublicKey?: string;
} {
...
async function collectWireguardProtectedRoutePlans(params: {
  ...
  const onlineDevices = new Map(
    params.bridge.listDevices().map((entry) => [entry.deviceId.trim(), entry] as const),
  );
...
export function createClawWRTTools(params: { bridge: ClawWRTBridge; logger?: Logger }): AnyAgentTool[] {
  ...
  return [
    ...
    }),
    {
      ...
      execute: async (_toolCallId, rawParams) => {
        ...
        if (args.mode === "selective") {
          ...
          if (routePlanFile) {
            const routePlanFileData = await readWireguardProtectedRoutePlanFile(routePlanFile);
            if (!routePlanFileData) {
              throw new Error(`routePlanFile is invalid or unreadable: ${routePlanFile}`);
            }
            const routePlan = routePlanFileData.routePlans.find((entry) => entry.deviceId === deviceId);
            if (!routePlan) {
    ...
    },
    {
      ...
      parameters: CollectWireguardProtectedRoutesSchema,
      execute: async (_toolCallId, rawParams) => {
        logToolInvocation(undefined, "clawwrt_collect_wireguard_protected_routes", rawParams);
        const args = rawParams as {
          deviceIds: string[];
          serverTunnelIp: string;
          timeoutMs?: number;
